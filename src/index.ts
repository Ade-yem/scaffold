    ...
    .prompt([
      {
        ...
        name: "choice",
        message:
          "There is an existing directory with the same name, do you want to overwrite it?",
        choices: ["yes", "no"],
      },
    ])
    .then(async (answer) => {
...
const createInternalFolders = async (
  directory: string,
  subDirectory: string,
) => {
  const res = path.join(directory, subDirectory);
  logger.warn("Creating internal folders " + res);
  fs.mkdirSync(res, { recursive: true }); // Ensure all parent directories are created
 ...
 */
const createFrontEndAndContract = async (
  projectPath: string,
  frontend: string,
  contract: string,
): Promise<string[]> => {
 ...
 */
const createFrontEndAndBackend = async (
  projectPath: string,
  frontend: string,
): Promise<string[]> => {
  const frontendPath = path.join(__dirname, "templates", frontend);
  ...
  try {
    ...
    switch (type) {
      case "smart contract + frontend":
        templatePath = await createFrontEndAndContract(
          packagePath,
          frontend as string,
          contract as string,
        );
      ...
      case "frontend + backend":
        templatePath = await createFrontEndAndBackend(
          packagePath,
          frontend as string,
        );
        break;
      default:
        templatePath = await createAllThree(
          packagePath,
          frontend as string,
          contract as string,
        );
